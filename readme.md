...
```warning
💡 Обратите внимание:
50 запросов в сек. - общее ограничение для всех одновременно выполняемых запросов от всех методов с одного client_id.
Многие методы могут вызывать исключение даже при 2-х запросах в сек, если отправлять такие запросы в течение нескольких секунд.
```
...
```warning
💡 Обратите внимание:
Логер кидает ошибки о превышении кол-ва запросов, при этом логикой планируется повторная отправка запроса, который инициировал исключение.
Т.е. краснота в консоли - это повод, чтобы обратить внимание на то, что желательно скорректировать настройки. Это штатная ситуация и программа при этом продолжает работу.
```

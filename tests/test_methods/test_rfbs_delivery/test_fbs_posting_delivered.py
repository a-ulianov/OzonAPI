...
class TestFbsPostingDelivered:
    """Тесты для метода fbs_posting_delivered."""

    @pytest.mark.asyncio
    async def test_fbs_posting_delivered(self, api, mock_api_request):
        """Тестирует метод fbs_posting_delivered."""
        ...
        response = await api.fbs_posting_delivered(request)

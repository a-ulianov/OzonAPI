...
class FbpDraftDropOffProductValidateRequest(BaseModel):
    """Схема запроса проверки списка товаров, которые склад партнёра может принять.
...
class FbpDraftDropOffProductValidateResponse(BaseModel):
    """Схема ответа проверки списка товаров для drop-off поставки.

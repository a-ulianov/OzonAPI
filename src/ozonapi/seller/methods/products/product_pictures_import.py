...
class ProductPicturesImportMixin(APIManager):
    ...
    async def product_pictures_import(
        ...
        Notes:
            ...
            • УСТАРЕЛО: поле `images360` удалено Ozon из запроса метода (live-спецификация от 2026-07-11).
              API его больше не принимает и игнорирует; поле сохранено в схеме `ProductPicturesImportRequest`
              для обратной совместимости и будет удалено в будущем мажорном релизе. Не передавайте его в новом коде.
            • Если вы хотите изменить состав или порядок изображений, получите информацию с помощью метода
              ...
              данные полей `images`, `color_image`, измените и дополните состав или порядок
